@tool
extends Node
## AutoSaveManager - Fully Automated Save/Load System for Godot 4
## 
...
const SAVE_VERSION := "1.0"  # Schema version for backward compatibility
...
const SKIP_PROPERTIES := [
	...
	"unique_name_in_owner", "owner", "editor_description"
]
...
func _connect_signals() -> void:
	# Connect to scene change signals to capture state transitions
	if Engine.get_main_loop():
		Engine.get_main_loop().connect("scene_changed", _on_scene_changed)


func _on_scene_changed(scene_root: Node) -> void:
	# Register nodes in the new scene for tracking
...
func _capture_node_state_recursive(node: Node, state_dict: Dictionary, root: Node, path_prefix: String = "") -> void:
	...
	var node_path := str(path_prefix, node.name) if path_prefix else node.name
	var node_uid := register_node(node)
...
func _serialize_node(node: Node) -> Dictionary:
	...
	var props := _serialize_object_properties(node)
	for key in props:
		if key not in state:
			state[key] = props[key]
...
func _deserialize_resource_from_dict(data: Dictionary) -> Resource:
	...
	var resource_type := data["_resource_type"]
	var resource_path := data.get("_resource_path", "")
...
func _apply_save_data(save_data: Dictionary) -> bool:
	...
	var registered_nodes := save_data.get("registered_nodes", {})
...
func _apply_scenes_data(scenes_data: Dictionary) -> void:
	...
	if scenes_data.has(scene_path):
		var scene_state := scenes_data[scene_path]
...
func _apply_node_states(root: Node, states: Dictionary) -> void:
	for node_path in states:
		var node_state := states[node_path]
...
func _copy_resource_properties(target: Resource, source: Resource) -> void:
	...
	var prop_list := source.get_property_list()
	for prop in prop_list:
		var prop_name := prop["name"]
		if prop_name in ["script", "resource_local_to_scene", "resource_name"]:
			continue
