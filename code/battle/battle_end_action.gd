	...
	match action_type:
		...
		ActionType.TRANSITION:
			if transition_scene != "":
				...
				Global.loading = true
				battle_engine.get_tree().change_scene_to_file(transition_scene)
				Global.loading = false
		ActionType.TEXTBOX:
			if textbox_data:
				var party_node = battle_engine.get_node_or_null("../player")
				if not party_node:
					party_node = battle_engine.get_tree().get_first_node_in_group("player")
				if party_node and party_node.has_method("make_textbox"):
					...
					await battle_engine.get_tree().create_timer(0.1).timeout
		ActionType.CUSTOM:
			if custom_script:
