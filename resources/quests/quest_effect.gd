...
func _execute_remove_quest() -> void:
	if QuestSystem:
		# Context should be the quest itself
		if context is Quest:
			QuestSystem.complete_quest(context)
...
func duplicate() -> QuestEffect:
	var new_effect = QuestEffect.new()
	new_effect.type = type
	new_effect.description = description
	new_effect.target_key = target_key
	new_effect.amount = amount
	new_effect.metadata = metadata.duplicate()
	new_effect.delay_seconds = delay_seconds
	new_effect.execute_immediately = execute_immediately
	new_effect.icon_override = icon_override
	new_effect.sound_effect = sound_effect
	return new_effect
